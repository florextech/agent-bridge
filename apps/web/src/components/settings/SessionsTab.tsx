...
export function SessionsTab({ botToken }: Readonly<{ botToken: string }>) {
  ...
  const agentPrompt = state.session ? `You have access to Agent Bridge, a notification system that lets you communicate with the user via Telegram and other channels.\n\n## Configuration\n- API: ${apiUrl}\n- Session: ${state.session.id}\n\n## MCP tools (preferred)\nIf agent-bridge MCP server is configured, use these tools:\n- notify — Send notification (type + summary)\n- check_responses — Check for unread messages\n- mark_read — Mark responses as read\n- wait_for_response — Wait until user responds (blocks until message arrives)\n\n## curl fallback\nNotify: curl -X POST ${apiUrl}/agent-events -H "Content-Type: application/json" -d '{"sessionId":"${state.session.id}","type":"EVENT_TYPE","payload":{"summary":"DESCRIPTION"}}'\nCheck: curl ${apiUrl}/agent-sessions/${state.session.id}/responses\nMark read: curl -X POST ${apiUrl}/agent-sessions/${state.session.id}/mark-read\n\n## Event types\n- task_started, task_completed, needs_review, needs_approval, error, test_results, message` : '';
